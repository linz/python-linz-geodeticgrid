...
class _bbox(object):
    ...
    def add(self, other):
        self.ymin = (
            other.ymin if self.ymin is None or self.ymin > other.ymin else self.ymin
        )
        self.ymax = (
            other.ymax if self.ymax is None or self.ymax < other.ymax else self.ymax
        )
        self.xmin = (
            other.xmin if self.xmin is None or self.xmin > other.xmin else self.xmin
        )
        self.xmax = (
            other.xmax if self.xmax is None or self.xmax < other.xmax else self.xmax
        )
...
class _packer(object):
    ...
    def writeschar(self, fh, value):
        fh.write(self.packschar(value))

    def writeshort(self, fh, value):
        fh.write(self.packshort(value))

    def writelong(self, fh, value):
        fh.write(self.packlong(value))

    def writedouble(self, fh, value):
        fh.write(self.packdouble(value))
...
class LinzDefModelBin(object):
    ...
    def analyzeModel(self):
        ...
        DefSeq = namedtuple(
            "DefSeq",
            "component description dimension zerobeyond steps grids timefuncs bbox",
        )
        ...
        class TimeEvent:
            ...
            def __str__(self):
                return "Time:{0} f0:{1} {2} f1:{3}".format(
                    self.time0, self.f0, self.time1, self.f1
                )
        ...
        for c in model.components():
            ...
            for m in c.spatialModel.models():
                if m.spatial_model != "llgrid":
                    raise RuntimeError(
                        "Cannot handle spatial model type " + spatial_model
                    )
            ...
            if len(events) > 0:
                ...
                for e in events:
                    if e0 and e0.time1.daysAfter(e.time0) > 0.001:
                        raise RuntimeError(
                            "Cannot handle overlapping time events in series"
                        )
                ...
                for i in reversed(list(range(len(time_model) - 1))):
                    ...
                    t1 = time_model[i + 1]
                    if (
                        abs(t0[0].daysAfter(t1[0])) < 0.001
                        and abs(t0[1] - t1[1]) < 0.00001
                    ):
                        time_model[i : i + 1] = []
    ...
    def write(self, binfile):
        ...
        for g in sorted(gridfiles):
            ...
            gf = LinzGrid(
                ...
                resolution=velocity_resolution
                if gdef.isvelocity
                else deformation_resolution,
            )
            gf.write(binfile)
        ...
        packer.writestring(
            binfile, model.metadata("model_name") + "(" + model.version() + ")"
        )
        ...
        for name, sequence, timefunc in defseq:
            ...
            for gridfile in sequence.grids:
                ...
                if timefunc.type == "VELOCITY":
                    ...
                    raise RuntimeError(
                        "Invalid timefunc type {0}".format(timefunc.type)
                    )
...
def main():
    ...
    argparser = argparse.ArgumentParser(
        "Build a LINZDEF deformation file from a published deformation model"
    )
